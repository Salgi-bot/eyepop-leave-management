// Vercel Edge Function — 최초 1회 Gist 생성
export const config = { runtime: 'edge' };

const GIST_API = 'https://api.github.com/gists';

const DEFAULT_EMPLOYEES = { employees: [], updatedAt: new Date().toISOString() };
const DEFAULT_SETTINGS = {
  ...
  vicePresidentEmail: 'rkhee@eyepopeng.com',
  updatedAt: new Date().toISOString()
};
const DEFAULT_REQUESTS = { requests: [], updatedAt: new Date().toISOString() };
const DEFAULT_CONFIRM_LOG = { confirmations: [], updatedAt: new Date().toISOString() };
...
export default async function handler(req) {
  ...
  if (existingId) {
    return json({
      status: 'exists',
      gistId: existingId,
      message: 'GIST_ID already configured in env vars'
    });
  }
  ...
  try {
    ...
    return json({
      ...
      message: `NEXT STEP: set GIST_ID=${data.id} in Vercel env then redeploy`
    });
  } catch (err) {
