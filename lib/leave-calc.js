...
// pending 일수 (대기 중인 신청 = 사용 예정으로 잔여에서 차감 → 이중 신청 방지)
export function calcPendingDays(employeeId, requests, year = new Date().getFullYear()) {
  return requests
    .filter(
      r =>
        r.employeeId === employeeId &&
...
export function calcRemaining(employee, requests, settings) {
  let total;
  // customLeaveDays가 명시되면 자동계산 무시 (수동 우선)
  // 회계년도 도중 도입 / 정년 촉진계약자 등 임의 입력 케이스
  if (employee.customLeaveDays != null && employee.customLeaveDays !== '') {
    total = Number(employee.customLeaveDays) || 0;
  } else {
    const mode = settings?.leaveCalcMode || 'legal_fiscal';
    if (mode === 'legal' || mode === 'legal_fiscal') total = calcLegalLeaveFiscal(employee.hireDate);
    else total = 0;
  }
  const preUsed = Number(employee.customUsedDays) || 0; // 시스템 도입 전 이미 사용한 일수 (선반영)
  const sysUsed = calcUsedDays(employee.id, requests);
  const pending = calcPendingDays(employee.id, requests);
